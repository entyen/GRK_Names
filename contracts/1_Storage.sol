...
contract NickNames is ERC721Enumerable, Pausable, GameOwner {
    using Strings for uint256;

    string private baseURI = "https://game.grk.pw/play/nfts/nick/";
    string public baseExtension = ".json";
    bytes8 [] Names;

    constructor() ERC721("NickName", "NAME") {
    ...
    /**
     * Load a char, the owner and its identifiable address (account) on the blockchain
     */
    function getChar(uint256 tokenId) public view returns (Nft memory) {
        address owner = ownerOf(tokenId);

        return Nft({
            owner: owner,
            tokenId: tokenId,
            name: Names[tokenId-1]
        });
    }

    /**
     * Get multiple chars for a single owner
     */
    function getChars(address account) public view returns (Nft[] memory) {
        uint balance = balanceOf(account);

        Nft[] memory accountChars = new Nft[](balance);
...
contract NftHolder is ERC165, IERC1155Receiver {
    NickNames private char;
    uint private id;

    constructor(NickNames _char, uint _id) {
        char = _char;
        id = _id;
    }
    ...
    function withdraw() public {
        require(char.ownerOf(id) == msg.sender, "You are not the owner");
